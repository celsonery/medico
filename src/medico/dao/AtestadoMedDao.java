 ...
 * @author celso
 */
public class AtestadoMedDao {

    Connection conn;

    public AtestadoMedDao() {
        conn = ConexaoSqlite.getConexao();
    }

    public int AdicionarModelo(AtestadoMedico aMed) {
        try {
    ...
    public int AlterarModelo(AtestadoMedico aMed) {
        try {
    ...
    public List<AtestadoMedico> ListasAtestadoMedico() {
        try {
            String sql = "select * from modelos where tipo = 'Medico'";

            PreparedStatement stmt = conn.prepareStatement(sql);

            ResultSet rs = stmt.executeQuery();

            List<AtestadoMedico> listAtMed = new ArrayList<>();

            while (rs.next()) {
                AtestadoMedico aMed1 = new AtestadoMedico();
